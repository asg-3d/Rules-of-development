...
1. Дизайн-макеты уникальных страниц и диалоговых окон (слои необходимо объединять в группу «Dialogs» поверх остальных слоев).Favicon, иконки закладок для мобильных устройств и пр. (в пикселях) (NOTE:  Для генерации можно воспользоваться утилитой (https://github.com/goooseman/webicon)):

    1. iPhone, iPad, iPad 3, iPhone 4, Android:
    ...
    4. Opera Speed Dial, Yandex Browser:
...
4. Техническое задание — описание поведения основных и второстепенных блоков:
    ...
    1. изменяемая/фиксированная ширина

    2. как тянутся колонки, поведение элементов при изменении ширины окна браузера

    3. поведение элементов при действиях пользователя например: наведение курсора мыши, активные/неактивные кнопки и пр.

    4. разные состояния блоков — для зарегистрированных/незарегистрированных пользователей и т. п.
...
7. Дизайн мобильной версии, если предполагается (см. с [пункта 1](#bookmark=id.w9ks9excotnu))

8. Комментарии SEO-специалиста о положении тегов, влияющих на [SEO-оптимизацию](#bookmark=id.hw0tw0hd7hjg) (NOTE:  Cм. «Требования SEO»)
...
4. Должны использоваться изображения следующих форматов png, gif, jpg и использоваться в соответствии с назначением:
    ...
    1. JPG — сохранение больших изображений с большим количеством цветов и оттенков.

    2. GIF — сохранение небольших изображениий типа значков и стрелок, а также повторяющихся фоновых изображений

    3. PNG — сохранение любых изображений с большими монотонными областями или небольших изображений, фоновых неповторяющихся изображениий, а также изображений с альфа-каналом
...
8. Следует показывать конец длинного блока, отображая в комментарии его class, id, name (и пр.) css-селектором, чтобы можно было легко найти конец или начало блока. Пример:

    1. <!-- .box -->  <div class="box">...</div><!-- /.box -->

    2. <!-- #content --><div id="content">...</div><!-- /#content -->

    3. <!-- [name="calculate-form"] --><form method="get" name="calculate-form"> ... </form><!-- /[name="calculate-form"] -->  
...
1. При верстке для документов следует использовать страндарт HTML5<!DOCTYPE HTML>
...
4. Приветствуется использование [стандартов семантической разметки](http://schema.org/) для поисковых систем (NOTE:  http://schema.org/, перевод — http://ruschema.org/)
...
5. Рекомендуется оформлять логически завершенный функционал как **[плагин**ы](http://docs.jquery.com/Plugins/Authoring#The_Basics) (NOTE:  http://docs.jquery.com/Plugins/Authoring#The_Basics), используя [фабрику](#bookmark=id.ojnbsol7fjin).
...
* **[lessCS**S](http://lesscss.org/) (NOTE:  http://lesscss.org/) — улучшенный CSS с переменными, "функциями", наследованием и вложенностью

* **[emme**t](http://emmet.io) (NOTE:  http://emmet.io) — быстрое написание структуры HTML/CSS в редакторе

* **[Tabifie**r](http://tools.arantius.com/tabifier) (NOTE:  http://tools.arantius.com/tabifier) — автоматическая расстановка отступов и переносов в HTML, CSS и JS

* **[Рецеп**т](http://matthewjamestaylor.com/blog/keeping-footers-at-the-bottom-of-the-page) (NOTE:  http://matthewjamestaylor.com/blog/keeping-footers-at-the-bottom-of-the-page) создания футера, «прижатого» к низу страницы

* **[Style Guide**s](http://developer.android.com/design/index.html) (NOTE:  http://developer.android.com/design/index.html) для приложений/сайтов под Андроид

* **[jsfiddl**e](http://jsfiddle.net/) (NOTE:  http://jsfiddle.net/) — хостинг-песочница для JS, CSS, HTML 

* **[TinyPN**G](http://tinypng.org) (NOTE:  http://tinypng.org) — сжималка PNG изображений без потерь

* **[Cufo**n](http://cufon.shoqolate.com/generate/) (NOTE:  http://cufon.shoqolate.com/generate/) — генерация кастомных шрифтов для веб
...
Необходимо дописать CSS таким образом, чтобы получилось отображение в браузере следующих видов:
